use shadow_config::{for_each_model_provider_slot, Config, CustomModelProviderConfig};
...
pub trait FamilyProviderFactory {
    fn create_provider(&self, alias: &str, key: Option<&str>, api_url: Option<&str>, options: &ModelProviderRuntimeOptions) -> anyhow::Result<Box<dyn ModelProvider>>;
}
...
impl FamilyProviderFactory for CustomModelProviderConfig {
    fn create_provider(&self, alias: &str, key: Option<&str>, api_url: Option<&str>, options: &ModelProviderRuntimeOptions) -> anyhow::Result<Box<dyn ModelProvider>> {
        let base_url = api_url.ok_or_else(|| {
            anyhow::Error::msg(
                "Custom model_provider required `url`"
            )
        })?;
        
        let mut p = crate::openai::OpenAiCompatibleModelProvider::new_with_vision(alias, "Custom", base_url, key, AuthStyle::Bearer, true);
...
fn apply_compat_options(mut p: OpenAiCompatibleModelProvider, opts: &ModelProviderRuntimeOptions) -> Box<dyn ModelProvider> {
    Box::new(p)
    // todo 
}

pub fn dispatch_family_factory(config: Option<&Config>, family: &str, alias: &str, key: Option<&str>, api_url: Option<&str>, options: &ModelProviderRuntimeOptions) -> anyhow::Result<Box<dyn ModelProvider>> {
