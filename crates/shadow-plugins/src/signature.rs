        ...
        (Some(sig), Some(pub_key)) => {
            ...
            match &result {
                VerificationResult::Valid { publisher_key } => {
                    ::shadow_log::record!(INFO, ::shadow_log::Event::new(module_path!(), ::shadow_log::Action::Note).with_attrs(::serde_json::json!({"plugin": plugin_name, "publisher_key": publisher_key.as_str()})), "plugin signature verified");
                    Ok(result)
                }
                VerificationResult::Untrusted => match mode {
                    ...
                    SignatureMode::Permissive => {
                        ::shadow_log::record!(WARN, ::shadow_log::Event::new(module_path!(), ::shadow_log::Action::Note).with_outcome(::shadow_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"plugin": plugin_name, "publisher_key": pub_key})), "plugin publisher key not trusted; loading in permissive mode");
                        Ok(result)
                    }
                ...
                VerificationResult::Invalid { reason } => match mode {
                    ...
                    SignatureMode::Permissive => {
                        ::shadow_log::record!(WARN, ::shadow_log::Event::new(module_path!(), ::shadow_log::Action::Note).with_outcome(::shadow_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"plugin": plugin_name, "reason": reason.as_str()})), "plugin signature invalid; loading in permissive mode");
                        Ok(result)
                    }
                    SignatureMode::Disabled => Ok(result),
                },
