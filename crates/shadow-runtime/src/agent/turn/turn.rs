...
use crate::agent::turn::provider_call::{ProviderCallOutcome, call_provider};
...
pub async fn run_tool_call_loop(tool_loop: ToolLoop<'_>) -> anyhow::Result<String> {
    ...
    for iteration in 0..max_iterations {
        let ProviderCallOutcome { chat_result, .. } =
            call_provider(&ctx, model_provider, model, history, None, false, iteration).await?;

        let (response_text) = match chat_result {
            Ok(resp) => (resp.text.as_deref().unwrap_or("").to_string()),
            Err(e) => return Err(e),
        };
...
pub fn is_model_switch_requested(error: &anyhow::Error) -> Option<(String, String)> {
    error
        .chain()
        .filter_map(|s| s.downcast_ref::<ModelSwitchRequested>())
