...
pub async fn run(
  ...
  allowed_tools: Option<Vec<String>>,
    ...
    let risk_profile = config.risk_profile_for_agent(agent_alias).with_context(|| format!("agents.{agent_alias}.risk_profile does not name a configured risk_profiles entry."))?.clone();
    ...
    let base = async move {
        ...
        if interactive && matches!(config.observability.backend, ObservabilityBackend::Prometheus) {
            // todo record
...
fn resolve_agent_for_turn(config: &Config, agent_alias: &str) -> anyhow::Result<AliasedAgentConfig> {
    let agent = config.resolved_agent_config(agent_alias)
