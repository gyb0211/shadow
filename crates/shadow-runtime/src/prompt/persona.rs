...
mod tests {
    ...
    fn test_builtin_fallback() {
        ...
        let personas = load_personas(&empty);
    ...
    fn test_config_overrides_builtin() {
        ...
        config.insert("helpful".to_string(), "覆盖的提示".to_string());
        let personas = load_personas(&config);
        let helpful = personas.iter().find(|p| p.name == "helpful").unwrap();
        assert_eq!(helpful.system_prompt, "覆盖的提示");
    }

    #[test]
    fn test_config_adds_custom() {
        let mut config = HashMap::new();
        config.insert("pirate".to_string(), "Arrr!".to_string());
        let personas = load_personas(&config);
        assert!(personas.iter().any(|p| p.name == "pirate"));
        assert!(personas.len() > 10);
    }

    #[test]
    fn test_get_persona_config_priority() {
        let mut config = HashMap::new();
        config.insert("custom_role".to_string(), "自定义提示".to_string());
        let p = get_persona("custom_role", &config).unwrap();
        assert_eq!(p.name, "custom_role");
        assert_eq!(p.system_prompt, "自定义提示");
    }

    #[test]
    fn test_get_persona_builtin_fallback() {
        let empty: HashMap<String, String> = HashMap::new();
        let p = get_persona("concise", &empty).unwrap();
    ...
    fn test_get_persona_not_found() {
        let empty: HashMap<String, String> = HashMap::new();
        assert!(get_persona("nonexistent", &empty).is_none());
    }
    ...
    fn test_section_render() {
        let empty: HashMap<String, String> = HashMap::new();
        let section = PersonaSection::from_name("concise", &empty);
        let text = section.render(&make_ctx());
        assert!(text.contains("[人格模式: concise]"));
    }

    #[test]
    fn test_section_from_name_fallback() {
        let empty: HashMap<String, String> = HashMap::new();
        let section = PersonaSection::from_name("unknown", &empty);
        assert_eq!(section.persona().name, "helpful");
    }

    #[test]
    fn test_list_personas_with_config() {
        let mut config = HashMap::new();
        config.insert("pirate".to_string(), "Arrr!".to_string());
        let names = list_personas(&config);
        assert!(names.contains(&"helpful".to_string()));
