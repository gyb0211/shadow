...
mod tests {
    ...
    #[test]
    fn test_empty_tools() {
        let text = build_prompt_guided_instructions(&[]);
        assert!(text.is_empty(), "空工具列表应返回空字符串");
    }

    /// 测试: 指令文本包含调用模板与工具列表
    #[test]
    fn test_instructions_format() {
        let tools = vec![
            make_tool("shell", "执行 shell 命令"),
            make_tool("file_read", "读取文件"),
        ];
        let text = build_prompt_guided_instructions(&tools);

        assert!(text.contains("<tool_call>"), "应包含调用模板");
        assert!(text.contains("</tool_call>"), "应包含闭合标签");
        assert!(text.contains("可用工具:"), "应包含工具列表标题");
        assert!(text.contains("1. shell - 执行 shell 命令"), "应包含工具1");
        assert!(text.contains("2. file_read - 读取文件"), "应包含工具2");
        assert!(text.contains("参数:"), "应包含参数信息");
    }
...
"#;
        let calls = parse_prompt_guided_response(text);
...
"#;
        let calls = parse_prompt_guided_response(text);
        assert_eq!(calls.len(), 1, "无效 JSON 块应被跳过");
        assert_eq!(calls[0].name, "shell");
    }
...
"#;
        let calls = parse_prompt_guided_response(text);
        assert!(calls.is_empty(), "缺少 name 字段应被跳过");
    }

    /// 测试: 缺少 arguments 时默认为 Null
    #[test]
    fn test_parse_missing_arguments_defaults_null() {
        let text = r#"
<tool_call>
{"name": "ping"}
</tool_call>
"#;
    ...
    /// 测试: 紧凑格式的 tool_call (无多余空白)
    #[test]
    fn test_parse_compact_call() {
        let text = r#"<tool_call>{"name":"shell","arguments":{"command":"echo hi"}}</tool_call>"#;
        let calls = parse_prompt_guided_response(text);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].name, "shell");
        assert_eq!(calls[0].arguments, json!({"command": "echo hi"}));
    }
}
